...
public class Locks1 {
    ...
    public static boolean TryLock(String clientId, String rootNode) throws InterruptedException, KeeperException{
        ...
        if (OwnTheLock(clientId, rootNode)){
            ...
            // block the client until the lock has changed
            cc.await();
            return true;
        }
    }
...
//     // key function for distributed lock
//     private static void getLockOrWait(String clientId, String rootNode) throws KeeperException, InterruptedException {
...
//         // check if the lock owner equals UUID
//         if(data!=null && new String(data).equalsIgnoreCase(clientId)){
//             System.out.println("Get a lock successfully! The expiration time is 60 seconds");
//             for (int i=0;i<60;i++){
//                 System.out.println("lock for "+ i + "seconds");
//                 Thread.sleep(1000);
//             }
